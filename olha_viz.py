# PB (12-01-2021): credit for this code goes to Olha Zolotarenko,
# who wrote her BA thesis on the subject of coherence visualisation.
#  This is just the conversion script to accomodate the format brat
#  (which the viz component was based on) expects.
# (private repo to be found here:
# https://github.com/zolotarenko/VisualizationOfCoherenceRelations)

# Algorithm
# Idea: internal.json ---> parse ---> exetrnal_brat.json
# ---> load in index.xhtml as variable
# 1. DocData
...
# var docData = {
#     // Our text of choice
#     text     : "blablabla. The snow was heavy. But we kept going.
#                 Until the sun came back. more blabla.",
...
#     relations : [
#     // Format:
#     [${ID}, ${TYPE}, [[${ARGNAME}, ${TARGET}], [${ARGNAME}, ${TARGET}]]]
...
def convert(textinput, jsoninput):
    ...
    # with open("/Users/apple/ba_visualization/data/olha_example_input.txt")\
    #     as f:
    ...
    for rel in input_data:
        # Iterating properties of very relation : key-value pairs in
        # the relation
        # E.g. "ID", "DocID", "Sense" , ...
        for rel_property_key, rel_property_value in rel.items():
            # Arguments and connectives have dict values with needed
            # "CharacterSpanList" in it
            ...
            if type(rel_property_value) == dict:
                for entity_key, entity_value in rel_property_value.items():
                    if entity_key == 'CharacterSpanList':
                        ...
                        # - [[300, 372]]] - normal case,
                        # simple continuous argument/connective
                        # - [[405, 457], [543, 590]] -
                        # discontinuous argument/connective
                        # - [] - empty argument/connective for implicit
                        # relations or

                        # WARNING Fix of the bug (22.3.21):
                        # the tested document contains empty arguments,
                        # which was filtered earlier
                        # and was causing the wrong alignment of the relations
                        # if entity_value:

                        # Create entity and add to the docData["entities"]
                        entity = ["E" + str(entities_counter_id),
                                  rel_property_key, entity_value]
    ...
    for rel in input_data:
        for rel_property_key, rel_property_value in rel.items():
            ...
            if rel_property_key == "Sense":
                ...
                name = str(relation_type[relations_counter_id - 1])\
                    + '.' + rel_property_value
