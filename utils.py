...
def get_discont_match_positions(lst, sublst):
    # known bug: this function only returns the first match.
    # Chances of multiple discontinuous connectives in one sentence are
    # low though.
    positions = []
    for item in sublst:
...
def get_chunks(x):
    # to be sure; probably input is always sorted already
    # though
    x = sorted(set(x))
    gaps = [[s, e] for s, e in zip(x, x[1:]) if s+1 < e]
    edges = iter(x[:1] + sum(gaps, []) + x[-1:])
    return list(zip(edges, edges))


def getPathToRoot(ptree, route):
    if ptree.parent() is None:
        ...
        if i == pos:
            ...
            if rs:
                if rs.label() == 'S':
                    ...
                    # assuming that there are no duplicates of the
                    # connective anymore at this level of detail:
                    leaves = parent.leaves()
                    connindex = leaves.index(ct.token)
                    remainder = [xj for xi, xj in enumerate(leaves)
                                 if xi >= connindex]
                    return remainder
            else:
                parent = pt[nodepos[:-1]].parent()
    ...
    for i, pair in enumerate(_sorted):
        if plain_tokens[0] == pair[1]:
            if len(plain_tokens) > 1:
                # second condition to accomodate stupid bracket
                # deletion requirement
                if plain_tokens[1] == _sorted[i+1][1]\
                        or plain_tokens[1] == _sorted[i+2][1]\
                        and _sorted[i+1][1] in '()':
                    anchor = _sorted[0][0]
                    if len(plain_tokens) == 2:
                        anchor = _sorted[i][0]
                    elif plain_tokens[2] == _sorted[i+2][1]\
                            or plain_tokens[2] == _sorted[i+3][1]\
                            and plain_tokens[1] in '()':
                        anchor = _sorted[i][0]
                    else:
                        pass
                    ret = []
                    for i in range(anchor, anchor + len(plain_tokens)):
                        ret.append(i)
                    return ret
            # in case of VP args it can sometimes be only 1 or 2 tokens
            # I guess
            else:
                anchor = _sorted[0][0]
                ret = []
...
def left_siblings(st, x):
    if st.left_sibling():
        x.append(st.left_sibling())
        x = left_siblings(st.left_sibling(), x)
    return x


def right_siblings(st, x):
    if st.right_sibling():
        x.append(st.right_sibling())
        x = right_siblings(st.right_sibling(), x)
    return x


def getLeaveNr(refcon, tree):
    leavenr = 0
    # this is not necessarily the case, since brackets are deleted
    # for parsing
    if tree.leaves()[refcon.sentenceTokenId] == refcon.token:
        leavenr = refcon.sentenceTokenId
    else:
        bracketnr = 0
        restored = False
        if re.search(r'[\(\)]', refcon.fullSentence):
            for char in ' '.join(
                    refcon.fullSentence.split()[:refcon.sentenceTokenId]):
                if char == '(' or char == ')':
                    bracketnr += 1
        if bracketnr:
            if tree.leaves()[refcon.sentenceTokenId-bracketnr] == refcon.token:
                restored = True
        # if this is not the case, something else went wrong locating
        # the token in the tree (any other special characters NLTK
        # treats differently during parsing?)
        assert restored
    return leavenr
...
def getFeaturesFromTreeDiscont(ptree, positions, reftoken):
    ...
    for i, node in enumerate(parentedTree.pos()):
        if i == positions[0] and node[0] == reftoken[0]:
            ...
            currPos = '_'.join([x[1] for i2, x in enumerate(parentedTree.pos())
                                if i2 in positions])
            features.append(currWord)
            features.append(currPos)
            ln = 'SOS' if i == 0 else parentedTree.pos()[i-1]
            rn = 'EOS' if positions[-1] == len(parentedTree.pos())\
...
def getFeaturesFromTreeCont(ptree, position, reftoken):
    '''Returns list with syntactic features'''
    ...
    if isinstance(position, int):  # single token
        for i, node in enumerate(parentedTree.pos()):
            if i == position and node[0] == reftoken:
                ...
                features.append(currPos)
                ln = 'SOS' if i == 0 else parentedTree.pos()[i-1]
                rn = 'EOS' if i == len(parentedTree.pos())-1\
        ...
        for i, node in enumerate(parentedTree.pos()):
            if i == position[0] and node[0] == reftoken[0]:
                currWord = '_'.join([x[0] for x in parentedTree.pos()[i:i+len(
                    reftoken)]])
                currPos = '_'.join([x[1] for x in parentedTree.pos()[i:i+len(
                    reftoken)]])
                features.append(currWord)
                features.append(currPos)
                ln = 'SOS' if i == 0 else parentedTree.pos()[i-1]
                rn = 'EOS' if i == len(parentedTree.pos()) - len(reftoken)\
