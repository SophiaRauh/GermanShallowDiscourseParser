...
def parseXML(dimlexml):
    ...
    for entry in tree.getroot():
        ...
        for orth in entry.find('orths').findall('orth'):
            ...
            t1 = 'single' if set(
                [x.get('type') for x in orth.findall('part')]) else 'phrasal'
        ...
        if non_connNode is not None:
            ...
            if non_connNode.text == '1':
                if 'freq' in non_connNode.attrib and 'anno_N'\
                        in non_connNode.attrib:
                    p = 1 - (float(non_connNode.get('freq')) / float(
                        non_connNode.get('anno_N')))
        ...
        for syn in syns:
            ...
            for sem in syn.findall('sem'):
                for sense in sem:
                    ...
                    if not freq == '0' and not freq == '' and not anno == '0'\
                            and not anno == '' and anno is not None:
                        # bug in lxml due to whitespace in attrib name,
                        # or by design?
                        pdtb3sense = sense.get('sense')
                        prob = 1 - (float(freq) / float(anno))
                        dl.addSense(pdtb3sense, prob)
                    elif freq is None and anno is None:
